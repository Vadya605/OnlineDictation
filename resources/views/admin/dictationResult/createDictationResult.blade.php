...
<div class="col-8 col-sm-6 offset-sm-3 offset-2">
    ...
    <form class="mt-5" action="{{ route('storeDictationResult') }}" method="POST">
        @csrf
        <div class="mb-3">
            <label for="dictation_id" class="form-label">Id диктанта *</label>
            <input type="text" class="form-control" id="dictation_id" name="dictation_id" value="{{ old('dictation_id') }}">
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" name="text_result" id="text_result" cols="30" rows="5">{{ old('text_result') }}</textarea>
        </div>
        <div class="mb-3 form-check date-time-selection">
            <input value="{{ old('date_time_result') }}" type="datetime-local" class="form-check-input" id="date_time_result" name="date_time_result">
